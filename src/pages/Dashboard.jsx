...
import { calculateStreak, isIncompleteToday, checkMilestone } from '../utils/habitUtils';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const updatedHabits = storedHabits.map(habit => {
      const streak = calculateStreak(habit.progressLog);
      ...
      if (milestone && !habit.milestoneShown?.includes(milestone)) {
        ...
        return {
          ...habit,
          streak,
          milestoneShown: [...(habit.milestoneShown || []), milestone],
        };
      }
      return { ...habit, streak };
    });
    ...
    const incompleteHabits = updatedHabits.filter(habit => isIncompleteToday(habit.progressLog));
    if (incompleteHabits.length > 0) {
      toast(
      (t) => (
        <div className="flex items-center">
        <span className="mr-4">
          You have {incompleteHabits.length} habit{incompleteHabits.length > 1 ? 's' : ''} to complete today!
        </span>
        <button
          onClick={() => toast.dismiss(t.id)}
          className="ml-auto bg-transparent text-gray-500 hover:text-gray-700 dark:text-gray-300 dark:hover:text-gray-100"
        >
          ✖
        </button>
        </div>
  ...
  return (
    <div className={`min-h-screen relative overflow-hidden ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          <h1 className="md:text-5xl text-2xl font-bold text-gray-900 dark:text-white mb-2">HabitSpark</h1>
