...
export default function HabitCard({ habit }) {
  ...
  const latestProgress = habit.progress !== undefined 
    ? habit.progress 
    : habit.progressLog?.length > 0 
      ? habit.progressLog[habit.progressLog.length - 1].progress 
      : 0;

  const createdAt = habit.createdAt || new Date(habit.id).toISOString();
  const updatedAt = habit.progressLog?.length > 0 
    ? habit.progressLog[habit.progressLog.length - 1].date 
    : createdAt;
    ...
    >
      <div className="p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          {habit.streak >= 10 && (
            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-yellow-400 text-gray-900">
